...
| `/dining/:slug` | `VenueDetailPage.tsx` (section=dining) | `shared/Button` `detail/DetailHeroBanner` `detail/DetailBreadcrumb` `detail/DetailSection` `detail/ContactRow` `detail/MarqueeGallery` `blocks/FaqAccordion` `blocks/Testimonials` `kids/KidsPartyPackages` `shared/CtaIcon` | 🟢 collection `restaurant` (by slug) — fallback to `frontend/src/data/subpages.ts` if not present in CMS. **`bottomCtas` is CMS-authoritative** (patch-2026-06-16): it is explicitly populated and merged with `??` (not `?.length ?`), so clearing the list in the CMS hides the section instead of resurrecting the static `subpages.ts` copy. All detail-page CTA links (`ctas`, `bottomCtas`, `imagePanels`, `cardSections`, `downloads`, markdown links) route through `isExternalHref()` so `/uploads/` + absolute URLs open as real `<a>` tabs even when the editor leaves the `isExternal` toggle off. |
