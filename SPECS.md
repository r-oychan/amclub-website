...
| `/membership/niche-group-membership` | `NicheGroupMembershipPage.tsx` | `detail/DetailHeroBanner` `detail/DetailBreadcrumb` `blocks/BlockRenderer` (priced-card-grid tier cards) `shared/CtaIcon` | 🟢 single-type `niche-group-membership-page` — heroImage + per-tier-card images editable in CMS (patch-2026-06-12-niche-group-images) |
