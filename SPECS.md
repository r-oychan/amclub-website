...
| Page / area | Status |
|---|---|
| `KidsPage` | ✅ 2026-06-16 — `QuadSection` → `kids-page.quadSection` (`shared.card-section`, 3 Quad cards) and `ChildSafetySection` → `kids-page.childSafety` (`shared.child-safety`: badge/heading/body/feature labels + bg image). Decorative inline SVG icons stay inline, matched to CMS feature labels by order. Components accept props and fall back to built-in defaults. Seed: `patch-2026-06-16-kids-quad-safety.mjs`. Controller POPULATE updated. |
| `GalleryPage`, `NewsPage` UI chrome | ✅ 2026-06-16 — "Load More" / "Read More" / "View Album" now read from `site-config` (`loadMoreLabel`/`readMoreLabel`/`viewAlbumLabel`) via the `useSiteCopy()` hook (code-level defaults guard against blank UI). Seed: `patch-2026-06-16-site-copy-labels.mjs`. (`WhatsOnPage` had no such strings.) |
| `fitness-facility` rich sections | ✅ 2026-06-16 — Tennis `imagePanels`, Aquatics `faq`+`promoCards`, Gym/Pilates `cardSections`, Bowling `faq` now CMS-driven (new flat fields + reused `shared.card-section`/`blocks.promo-card-grid`/`shared.faq-pair`). Restaurant `promoCards` (Gourmet Pantry) likewise. Coach grids render from the per-discipline coach collections (`tennis-coaches`/`gym-trainers`/`pilates-instructors`/`aquatics-coaches`) via VenueDetailPage, not subpages.ts. Seeds: `seed-fitness-detail-sections.mjs`, `patch-2026-06-15-tennis-imagepanels.mjs`, `patch-2026-06-15-gourmet-promocards.mjs`. **Root-cause note:** these were invisible not because of the DB but because the custom controllers' hardcoded `POPULATE` maps omitted the new fields — see `docs/troubleshooting.md` "controller POPULATE map drift". |
...
| `HeroCarousel.tsx` | `blocks.hero` (slides) | Uses `slides` repeatable + `autoPlayInterval`. `mobileFitMedia` (boolean, default false): on mobile (<768px) shows each slide's media in full — capped to viewport width/height, letterboxed on brand navy, title/CTA/dots flow below the image — instead of full-screen cover-crop. Desktop is unaffected. Touch: horizontal swipe (50px+, more horizontal than vertical) moves next/previous slide; taps still activate links/CTAs. |
