/**
 * Walks an entry to find every Strapi-uploaded document file reachable
 * from it via:
 *   1. media-relation attributes (recurses into components + dynamic zones)
 *   2. configured `mediaUrlPaths` (dot-paths to string URLs pointing at /uploads/)
 *
 * Returns deduped + stably-ordered file rows so the sync log is reproducible.
 */
