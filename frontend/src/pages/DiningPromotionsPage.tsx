...
/**
 * Tag is the URL-anchor key: `'club-wide'` for club-wide promos, otherwise
 * the linked restaurant's slug. Dynamic — derived per request from data.
 */
type Tag = string;
const CLUB_WIDE: Tag = 'club-wide';

interface StrapiPromotionRestaurant {
  slug: string;
  name?: string;
  menuUrl?: string | null;
  order?: number | null;
...
interface StrapiPromotion {
  ...
  slug: string;
  summary?: string;
  restaurant?: StrapiPromotionRestaurant | null;
  isClubWide?: boolean;
...
function tagFor(p: StrapiPromotion): Tag | null {
  if (p.isClubWide) return CLUB_WIDE;
  return p.restaurant?.slug ?? null;
}

/** Display name shown in the sidebar pill + above each promo card. */
function labelFor(tag: Tag, promos: StrapiPromotion[]): string {
  if (tag === CLUB_WIDE) return 'Club-wide';
  const restaurant = promos.find((p) => p.restaurant?.slug === tag)?.restaurant;
  return restaurant?.name ?? tag;
}
...
// Group promotions by tag (club-wide first, then restaurants by their
// `order` field ascending, then name). Tags whose group ends up empty are
// dropped from the map so the sidebar doesn't render dead pills.
function groupPromotions(promotions: StrapiPromotion[]): Map<Tag, StrapiPromotion[]> {
  const groups = new Map<Tag, StrapiPromotion[]>();
  for (const p of promotions) {
    const tag = tagFor(p);
    if (!tag) continue; // promo with no restaurant and not club-wide → skip
    if (!groups.has(tag)) groups.set(tag, []);
    groups.get(tag)!.push(p);
  }
  // Re-key by deterministic order.
  const ordered = new Map<Tag, StrapiPromotion[]>();
  const keys = Array.from(groups.keys()).sort((a, b) => {
    if (a === CLUB_WIDE) return -1;
    if (b === CLUB_WIDE) return 1;
    const ra = groups.get(a)![0].restaurant;
    const rb = groups.get(b)![0].restaurant;
    const oa = ra?.order ?? 0;
    const ob = rb?.order ?? 0;
    if (oa !== ob) return oa - ob;
    return (ra?.name ?? a).localeCompare(rb?.name ?? b);
  });
  for (const key of keys) ordered.set(key, groups.get(key)!);
  return ordered;
}
...
export default function DiningPromotionsPage() {
  ...
  // Derive display state: user-clicked > URL hash > first available tag.
  ...
  // `#promo-<slug>` hash. Pure side-effect — no state writes, the active
  // tag is already derived from the hash above.
  useEffect(() => {
    if (!loaded || !hashTag) return;
    // Wait one frame so refs have attached before scrolling.
    requestAnimationFrame(() => {
      const el = sectionRefs.current.get(hashTag);
      if (!el) return;
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (visible[0]) {
          const tag = visible[0].target.getAttribute('data-tag');
          if (tag) {
            setActiveTag(tag);
                    ...
                    return (
                      <li key={tag}>
                        {isActive ? (
                          ...
                          >
                            <span>{labelFor(tag, groups.get(tag) ?? [])}</span>
                            <ArrowRight size={18} />
                          </button>
                        ) : (
                          <button
                      ...
                      >
                        <div className="flex flex-col gap-y-16">
                          {items.map((p) => {
                            ...
                            const menuHref = p.restaurant?.menuUrl ?? undefined;
                            return (
                              <article key={p.documentId} className="max-w-[680px] mx-auto w-full">
                                ...
                                <p className="mt-5 text-[13.6px] font-bold uppercase tracking-[0.08em] text-accent">
                                  {p.isClubWide ? 'Club-wide' : (p.restaurant?.name ?? tag)}
                                </p>
